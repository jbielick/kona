 ...
 */
_.extend(Kona.prototype, require(join(__dirname, 'kona', 'logger')));
_.extend(Kona.prototype, require(join(__dirname, 'kona', 'watch')));
_.extend(Kona.prototype, require(join(__dirname, 'kona', 'mixins')));
_.extend(Kona.prototype, require(join(__dirname, 'kona', 'hooks')));
_.extend(Kona.prototype, {
  ...
  configure: function* () {
    ...
    yield this.hookFor('beforeConfigure');
  ...
  loadMiddleware: function () {
    ...
    this.middlewarePaths.forEach(function (mwPath) {
      ...
      debug(format('mounted middleware: %s', mwPath));
   ...
   */
  mountBaseModules: function () {
    this.Controller = {
      Request: require(this._root.join('lib', 'request-controller')),
      Base: require(this._root.join('lib', 'abstract-controller')),
      Abstract: require(this._root.join('lib', 'abstract-controller'))
    };
  },
  ...
  console: function (options) {
    ...
    var nullStream = fs.createReadStream('/dev/null');

    return repl.start({
      prompt: format('kona~%s > ', this.version),
      useColors: true,
      input: this.env === 'test' ? nullStream : process.stdin,
      output: this.env === 'test' ? nullStream : process.stdout
    });
