...
module.exports = (function(root) {
  ...
  Kona.prototype.bootstrap = function(program, env) {
    ...
    this.worker = !!(program && program.worker);
    ...
    this.log = new (winston.Logger)({
      transports: [
        new (winston.transports.File)({ filename: this.root.join('log', this.env + '.log') })
      ]
    });
  ...
  Kona.prototype.mount = function() {
    ...
    this.Controller = {
      ...
      Api: require(this._root.join('lib', 'api', 'api-controller'))
    };
    // expose base model
  };
  ...
  Kona.prototype.loadObjects = function(dir, strip) {
    ...
    read(dir).forEach(function(moduleName) {
      ...
      try {
        ...
        throw new Error(util.format("Failed to load %s: %s \n %s \n", moduleName, e.message, e.stack));
      }
    });
    return objects;
  };
